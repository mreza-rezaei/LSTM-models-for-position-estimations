...
y_data=mat['MSTrain'][:,64:65].astype('float32')
...
```
    ...
    <matplotlib.text.Text at 0x7f19c9c65f28>
...
```
    ...
    Saved trained model at /home/reza/DL/BrainProject/yousefi/saved_models/M1UE2Y.h5 
...
y_data=mat['MSTest'][:,64:65].astype('float32')
...
for i in range(y_test.shape[0]):
    #rmsX[i] = np.mean(distance(y_test[i], y_predic[i]))
    rmsY[i] = np.mean(distance(y_test[i], y_predic[i]))
...
plt1=plt.plot(y_predic[:],'b',label="EstimatedTestPath-Y")


plt.plot(y_test[:],'r',label="TestPath-Y")
...
plt.plot(y_test[:],'r',label="TestPath-X")
...
a=np.zeros(y_test.shape[0])
for i in range(y_test.shape[0]):
   # rmsX[i] = np.mean(distance(y_train[i], y_predic[i]))
    rmsY[i] = np.mean(distance(y_train[i], y_predic[i]))
...
plt.plot(y_train[:],'r',label="TrainPath-Y")
...
plt1=plt.plot(y_predic[:],'b',label="EstimatedTrainPath-X")


plt.plot(y_train[:],'r',label="TrainPath-X")
